...
declare var Valine: any;
declare var Waline: any;
...
declare var lgThumbnail: any;
declare var lgZoom: any;
...
class Util {
    static forEach(elements: any, handler: any) {
        elements = elements || [];
    ...
    static animateCSS(element: any, animation: any, reserved?: any, callback?: any) {
...
class Theme {
    ...
    newScrollTop: number;
    oldScrollTop: number;
    scrollEventSet: Set<any>;
    resizeEventSet: Set<any>;
    switchThemeEventSet: Set<any>;
    clickMaskEventSet: Set<any>;
    _menuMobileOnClickMask: any;
    _tocOnScroll: any;
    _commentOnScroll: any;
    _mapboxArr: any[];
    _mapboxOnSwitchTheme: any;
    _utterancesOnSwitchTheme: any;
    _giscusOnSwitchTheme: any;
    _resizeTimeout: any;

    _searchMobile: any;
    _searchMobileOnClickMask: any;
    _searchMobileOnce: any;
    _searchDesktop: any;
    _searchDesktopOnClickMask: any;
    _searchDesktopOnce: any;
    _index: any;
    _indexData: any;
    _algoliaIndex: any;
    _mermaidOnSwitchTheme: any;
    _echartsOnSwitchTheme: any;
    _echartsArr: any[];
    _echartsOnResize: any;
    ...
    constructor() {
        ...
        if ((window as any).objectFitImages) objectFitImages();
    }
        ...
        Util.forEach(document.querySelectorAll('[data-svg-src]'), $icon => {
            fetch($icon.getAttribute('data-svg-src'))
                ...
                .then(svg => {
                    ...
                    const $svg = $temp.firstChild as any;
    ...
    initSearch() {
        ...
        const $searchInput = document.getElementById(`search-input-${suffix}`) as any;
                    ...
                    if (searchConfig.type === 'lunr') {
                        const search = () => {
                            ...
                            this._index.search(query).forEach(({ ref, matchData: { metadata } }) => {
                                ...
                                let position = 0;
                                Object.values(metadata).forEach(({ content }: any) => {
                                    if (content) {
                        ...
                        if (!this._index) {
                            fetch(searchConfig.lunrIndexURL)
                                ...
                                .then(data => {
                                    const indexData = {};
                                    this._index = lunr(function (this: any) {
    ...
    initTypeit() {
        if (this.config.typeit) {
            ...
            Object.values(typeitConfig.data).forEach((group: any) => {
                const typeone = (i) => {
                    const id = group[i];
        ...
        if (this.config.comment) {
            ...
            if (this.config.comment.utterances) {
                ...
                this._utterancesOnSwitchTheme = this._utterancesOnSwitchTheme || (() => {
                    ...
                    const iframe = document.querySelector('.utterances-frame') as any;
    ...
    onScroll() {
        ...
        if (document.getElementById('comments')) {
            const $viewComments = document.getElementById('view-comments') as HTMLAnchorElement;
            if ($viewComments) {
                $viewComments.href = `#comments`;
                if ($viewComments.parentElement) {
